...
def rm_duplicate_lines(filepath, confirm=True):
...
def rm_less_than(n, filepath, confirm=True):
...
def main():
    ...
    while True:
        try:
            ...
            exit('Abort!')
    if option == 1:
        rm_duplicate_lines(get_path())
    else:
        rm_less_than(int(input("Less then?: ")), get_path())
